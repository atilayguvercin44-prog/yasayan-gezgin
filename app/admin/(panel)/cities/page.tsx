...
export default function AdminCitiesPage() {
  ...
  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`"${name}" silinecek. Devam etmek istiyor musunuz?`)) return
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="font-serif text-4xl font-semibold text-[#1C1C1C]">Şehirler</h1>
          <p className="text-sm text-[#9A9A9A] font-sans mt-1">{cities.length} kayıt</p>
        </div>
        <Link
      ...
      <div className="bg-white rounded-2xl shadow-[0_2px_16px_rgba(0,0,0,0.06)] overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-20">
            <Building2 size={32} className="text-[#E8E2DA] mx-auto mb-3" />
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#F0EBE3]">
                <th className="text-left px-6 py-4 text-xs font-medium text-[#9A9A9A] uppercase tracking-wider font-sans">Şehir</th>
              ...
              {cities.map((city) => (
                <tr key={city.id} className="hover:bg-[#FAF7F2] transition-colors">
                  <td className="px-6 py-4">
                    <div>
                      <span className="font-sans text-sm font-medium text-[#1C1C1C]">{city.name}</span>
                      <code className="ml-2 text-xs bg-[#F0EBE3] text-[#6B6B6B] px-1.5 py-0.5 rounded">{city.slug}</code>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <span className="text-sm text-[#6B6B6B] font-sans">
                      {(city.countries as { name: string } | undefined)?.name ?? '—'}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    {city.published ? (
